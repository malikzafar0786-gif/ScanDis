...
class _DocumentDetailScreenState extends State<DocumentDetailScreen> {
  ...
  Future<void> _exportAs(dynamic doc, String choice) async {
    ...
    try {
      switch (choice) {
        ...
        case 'jpeg':
          final files = await _exportService.getShareablePageFiles(
            doc.pageImagePaths,
            format: ExportImageFormat.jpeg,
          );
          await Share.shareXFiles(files.map((p) => XFile(p)).toList(), text: doc.title);
        case 'png':
          final files = await _exportService.getShareablePageFiles(
            doc.pageImagePaths,
          ...
          await Share.shareXFiles(files.map((p) => XFile(p)).toList(), text: doc.title);
        case 'zip':
          final zipPath = await _exportService.exportPagesAsZip(doc.title, doc.pageImagePaths);
          await Share.shareXFiles([XFile(zipPath)], text: '${doc.title} — all pages (ZIP)');
        case 'text':
  ...
  Future<void> _openInEditor(dynamic doc) async {
    ScanFilter? initialFilter;
    for (final f in ScanFilter.values) {
      if (f.name == doc.appliedFilter) {
        initialFilter = f;
        break;
      }
    }
    await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => EditDocumentScreen(
          pages: List<String>.from(doc.pageImagePaths),
    ...
    // The document may have changed (new pages/filter/PDF) — reload this
    // screen's data from the provider so Preview/OCR reflect the edit.
    if (mounted) setState(() {});
  }
            ...
            if (_exporting)
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (context) => [
                  ...
                  const PopupMenuItem(
                      value: 'jpeg', child: ListTile(leading: Icon(Icons.image_outlined), title: Text('Share as JPEG'))),
                  const PopupMenuItem(
                      value: 'png', child: ListTile(leading: Icon(Icons.image_outlined), title: Text('Share as PNG'))),
                  const PopupMenuItem(
                      value: 'text', child: ListTile(leading: Icon(Icons.text_snippet_outlined), title: Text('Text (.txt)'))),
                  const PopupMenuDivider(),
                  const PopupMenuItem(
                      value: 'zip', child: ListTile(leading: Icon(Icons.folder_zip_outlined), title: Text('Download All (ZIP)'))),
                ],
              ),
          ],
        ),
