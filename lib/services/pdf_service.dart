...
class PdfService {
  ...
  /// Generates a plain text-based PDF (not from images) — used for the
  /// Paper Maker feature and any other AI-generated text document.
  Future<String> generateTextPdf(String title, String content, {String? fileName}) async {
    final pdf = pw.Document();
    pdf.addPage(
      pw.MultiPage(
        build: (context) => [
    ...
    final dir = await getApplicationDocumentsDirectory();
    final name = fileName ?? title.replaceAll(RegExp(r'[^a-zA-Z0-9_\-]'), '_');
    final outPath = '${dir.path}/$name.pdf';
